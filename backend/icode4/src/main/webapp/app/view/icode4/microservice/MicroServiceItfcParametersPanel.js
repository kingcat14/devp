Ext.define('AM.view.icode4.microservice.MicroServiceItfcParametersPanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'icode4.microservice.MicroServiceItfcParametersPanel'
    ,title: '微服务接口参数'
    ,initComponent: function() {
        ...
        Ext.apply(me, {
            ...
            ,columns: [
                ...
                ,{
                    xtype: 'gridcolumn'
                    ,dataIndex: 'microServiceItfcId'
                    ,renderer: function(value, metaData, record, rowIndex, colIndex, store, view) {
                        return record.get("microServiceItfcName")?record.get("microServiceItfcName"):'';
                    }
    ...
    ,onSimpleSearchButtonClick: function(button, e, options) {
        ...
        var searchCondition = {viewIndex:simpleSearchField.getValue()}
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.icode4.microservice.MicroServiceItfcParameters', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新微服务接口参数');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改微服务接口参数信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除微服务接口参数成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改微服务接口参数信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        ...
        if(!addWindow||addWindow.isHidden()){
            ...
            addWindow = Ext.create('AM.view.icode4.microservice.MicroServiceItfcParametersAddWindow',{store:me.getStore()});
    ...
    ,showEditWindow: function(model, targetComponent) {
        ...
        if(!editWindow||editWindow.isHidden()){
            ...
            editWindow = Ext.create('AM.view.icode4.microservice.MicroServiceItfcParametersEditWindow',{store:me.getStore()});
    ...
    ,showDetailWindow: function(model, targetComponent) {
        ...
        if(!detailWindow||detailWindow.isHidden()){
            ...
            detailWindow = Ext.create('AM.view.icode4.microservice.MicroServiceItfcParametersDetailWindow',{store:me.getStore()});
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        if(!searchWindow){
            ...
            searchWindow = Ext.create('AM.view.icode4.microservice.MicroServiceItfcParametersSearchWindow');
