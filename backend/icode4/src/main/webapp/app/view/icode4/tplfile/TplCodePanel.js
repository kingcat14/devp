Ext.define('AM.view.icode4.tplfile.TplCodePanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'icode4.tplfile.TplCodePanel'
    ,title: '文件模板'
    ,initComponent: function() {
        ...
        Ext.apply(me, {
            ...
            ,columns: [
                ...
                ,{
                    xtype: 'gridcolumn'
                    ,dataIndex: 'acceptModelTypeId'
                    ,renderer: function(value, metaData, record, rowIndex, colIndex, store, view) {
                        return record.get("acceptModelTypeName")?record.get("acceptModelTypeName"):'';
                    }
                ...
                ,{
                    ...
                    ,dataIndex: 'tplSetId'
                    ,renderer: function(value, metaData, record, rowIndex, colIndex, store, view) {
                        return record.get("tplSetName")?record.get("tplSetName"):'';
                    }
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.icode4.tplfile.TplCode', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新文件模板');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改文件模板信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除文件模板成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,showAddWindow: function(model, targetComponent) {
        ...
        if(!addWindow||addWindow.isHidden()){
            ...
            addWindow = Ext.create('AM.view.icode4.tplfile.TplCodeAddWindow',{store:me.getStore()});
    ...
    ,showEditWindow: function(model, targetComponent) {
        ...
        if(!editWindow||editWindow.isHidden()){
            ...
            editWindow = Ext.create('AM.view.icode4.tplfile.TplCodeEditWindow',{store:me.getStore()});
    ...
    ,showDetailWindow: function(model, targetComponent) {
        ...
        if(!detailWindow||detailWindow.isHidden()){
            ...
            detailWindow = Ext.create('AM.view.icode4.tplfile.TplCodeDetailWindow',{store:me.getStore()});
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        if(!searchWindow){
            ...
            searchWindow = Ext.create('AM.view.icode4.tplfile.TplCodeSearchWindow');
