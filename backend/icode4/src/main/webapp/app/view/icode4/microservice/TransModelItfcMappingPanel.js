Ext.define('AM.view.icode4.microservice.TransModelItfcMappingPanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'icode4.microservice.TransModelItfcMappingPanel'
    ,title: '传输对象接口映射'
    ...
    ,onSimpleSearchButtonClick: function(button, e, options) {
        ...
        var searchCondition = {transModelId:simpleSearchField.getValue()}
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.icode4.microservice.TransModelItfcMapping', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新传输对象接口映射');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改传输对象接口映射信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除传输对象接口映射成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改传输对象接口映射信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        ...
        if(!addWindow||addWindow.isHidden()){
            ...
            addWindow = Ext.create('AM.view.icode4.microservice.TransModelItfcMappingAddWindow',{store:me.getStore()});
    ...
    ,showEditWindow: function(model, targetComponent) {
        ...
        if(!editWindow||editWindow.isHidden()){
            ...
            editWindow = Ext.create('AM.view.icode4.microservice.TransModelItfcMappingEditWindow',{store:me.getStore()});
    ...
    ,showDetailWindow: function(model, targetComponent) {
        ...
        if(!detailWindow||detailWindow.isHidden()){
            ...
            detailWindow = Ext.create('AM.view.icode4.microservice.TransModelItfcMappingDetailWindow',{store:me.getStore()});
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        if(!searchWindow){
            ...
            searchWindow = Ext.create('AM.view.icode4.microservice.TransModelItfcMappingSearchWindow');
