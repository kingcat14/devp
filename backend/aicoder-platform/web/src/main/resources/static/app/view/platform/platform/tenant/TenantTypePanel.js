Ext.define('AM.view.platform.platform.tenant.TenantTypePanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'platform.platform.tenant.TenantTypePanel'
    ,title: '租户类型'
    ,requires: [
        'AM.view.platform.platform.tenant.TenantTypeController'
        ,'AM.view.platform.platform.tenant.TenantTypeAddWindow'
        ,'AM.view.platform.platform.tenant.TenantTypeEditWindow'
        ,'AM.view.platform.platform.tenant.TenantTypeSearchWindow'
        ,'AM.view.platform.platform.tenant.TenantTypeDetailWindow'
    ]
    ...
    ,initComponent: function() {
        ...
        me.add({xtype:'platform.platform.tenant.TenantTypeAddWindow',reference:'tenantTypeAddWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'platform.platform.tenant.TenantTypeEditWindow',reference:'tenantTypeEditWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'platform.platform.tenant.TenantTypeSearchWindow',reference:'tenantTypeSearchWindow',listeners:{saved:'doSearch'}})
        me.add({xtype:'platform.platform.tenant.TenantTypeDetailWindow',reference:'tenantTypeDetailWindow'})
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.platform.platform.tenant.TenantType', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新租户类型');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改租户类型信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除租户类型成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改租户类型信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        var me = this;
        var addWindow = me.lookupReference('tenantTypeAddWindow');
    ...
    ,showEditWindow: function(model, targetComponent) {
        var me = this;
        var editWindow = me.lookupReference('tenantTypeEditWindow');
    ...
    ,showDetailWindow: function(model, targetComponent) {
        var me = this;
        var detailWindow = me.lookupReference('tenantTypeDetailWindow');
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        var searchWindow = me.lookupReference('tenantTypeSearchWindow');
