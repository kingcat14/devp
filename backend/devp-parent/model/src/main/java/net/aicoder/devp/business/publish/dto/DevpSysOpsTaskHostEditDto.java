 ...
 * @author icode
 */
@ApiModel(value = "修改部署主机节点使用的DTO")
public class DevpSysOpsTaskHostEditDto {
	...
	/**元素类型*/
	@ApiModelProperty(value = "元素类型", required = false, notes = "[元素类型]-SYS_DPY_OPS_TASK_HOST")
	...
	/**主机编号*/
	@ApiModelProperty(value = "主机编号", required = false, notes = "[主机编号]")
	private Long hostRid;


	/**执行环境编号*/
	@ApiModelProperty(value = "执行环境编号", required = false, notes = "[执行环境编号]")
	private Long envRid;


	/**是否部署*/
	@ApiModelProperty(value = "是否部署", required = false, notes = "[是否部署]-0-不需部署；1-部署")
