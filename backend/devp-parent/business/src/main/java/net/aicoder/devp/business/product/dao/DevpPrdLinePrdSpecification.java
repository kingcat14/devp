...
public class DevpPrdLinePrdSpecification implements Specification<DevpPrdLinePrd>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpPrdLinePrd> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpPrdLinePrd.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpPrdLinePrd.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
