...
public class DevpSysIdeRefSpecification implements Specification<DevpSysIdeRef>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpSysIdeRef> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpSysIdeRef.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpSysIdeRef.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
