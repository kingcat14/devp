...
public class DevpSysDpyHostSpecification implements Specification<DevpSysDpyHost>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpSysDpyHost> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpSysDpyHost.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpSysDpyHost.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
