...
public class DevpSysOpsPipeNodeSpecification implements Specification<DevpSysOpsPipeNode>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpSysOpsPipeNode> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpSysOpsPipeNode.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpSysOpsPipeNode.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
