...
public class DevpSysOpsTaskDockerSpecification implements Specification<DevpSysOpsTaskDocker>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpSysOpsTaskDocker> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpSysOpsTaskDocker.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpSysOpsTaskDocker.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
