...
public class DevpSysDpyCmpExecenvSpecification implements Specification<DevpSysDpyCmpExecenv>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<DevpSysDpyCmpExecenv> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(DevpSysDpyCmpExecenv.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(DevpSysDpyCmpExecenv.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
