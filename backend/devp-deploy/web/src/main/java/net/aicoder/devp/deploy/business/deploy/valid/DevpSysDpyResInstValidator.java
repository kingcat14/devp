...
public class DevpSysDpyResInstValidator implements Validator {
	...
	public void validateDevpSysDpyResInstAddDto(DevpSysDpyResInstAddDto devpSysDpyResInst, Errors errors) {
		...
		if (null == devpSysDpyResInst.getSchemeRid() ) {
			errors.rejectValue(DevpSysDpyResInst.PROPERTY_SCHEME_RID, "EMPTY_"+DevpSysDpyResInst.PROPERTY_SCHEME_RID, "部署方案编号不能为空");
		}
		...
		if(StringUtils.length(devpSysDpyResInst.getAccessAddr()) > 255){
			errors.rejectValue(DevpSysDpyResInst.PROPERTY_ACCESS_ADDR,null,"访问地址最长255个字符");
		}
