Ext.define('AM.view.speedcloud.pipeline.template.PipelineTemplateTaskEditPanel', {
    ...
    ,requires:[
        ...
        ,'AM.model.speedcloud.pipeline.template.PipelineTemplateTaskAction'
        ,'AM.view.speedcloud.pipeline.template.PipelineTemplateTaskActionEditPanel'
        ,'AM.view.speedcloud.pipeline.template.PipelineTemplateTaskEditController'
        ,'AM.model.speedcloud.pipeline.template.PipelineTemplateTaskParam'
    ]
    ,layout: 'border'
    // ,title: '修改新任务'
    ,maximizable: true
    ...
    ,bodyPadding:5
    ,bind:{title:'任务模板{record.name}'}
    // ,bind:{title:'任务名称:{title}'}
    ,bodyCls: 'app-dashboard'
    ,controller: 'speedcloud.pipeline.template.PipelineTemplateTaskEditController'
    // ,viewModel:{data:{}}
    ,initComponent: function () {

        var me = this;

        var pipelineTemplateTaskTaskTypeStore = Ext.create("AM.store.common.SimpleConfigStore")
        pipelineTemplateTaskTaskTypeStore.proxy.isSynchronous = true;
        pipelineTemplateTaskTaskTypeStore.proxy.extraParams={searchCondition:{configType:'PIPELINETASK-TASKTYPE'}};
        pipelineTemplateTaskTaskTypeStore.load();
                    ...
                    , items:[
                        {
                            ...
                            ,dockedItems:[
                                {
                                    ...
                                    ,items:{
                                        ...
                                        ,handler:function(){
                                            ...
                                            var action = Ext.create('AM.model.speedcloud.pipeline.template.PipelineTemplateTaskAction', {name:'执行Shell命令', memo:'执行Shell命令'});
                                            ...
                                            var panel = Ext.create('AM.view.speedcloud.pipeline.template.PipelineTemplateTaskActionEditPanel', {viewModel:{data:{record:action}}, frame:false});
                                            action.panel = panel;
                            ...
                            ,listeners:{
                                rowclick:function(table, record, element, rowIndex, e, eOpts){
                                    ...
                                    if(!record.dropped) {
                                        if(!record.panel){
                                            record.panel = Ext.create('AM.view.speedcloud.pipeline.template.PipelineTemplateTaskActionEditPanel'
                                                , {
                            ...
                            ,items:[
                                {
                                    ...
                                    ,items:[
                                        ...
                                        ,{
                                            xtype: 'combobox'
                                            ,store: pipelineTemplateTaskTaskTypeStore
                                            ,typeAhead:false
                                            ,editable:false
