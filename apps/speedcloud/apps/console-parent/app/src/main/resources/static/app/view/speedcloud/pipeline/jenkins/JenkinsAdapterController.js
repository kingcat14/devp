Ext.define('AM.view.speedcloud.pipeline.jenkins.JenkinsAdapterController', {
	extend: 'Ext.app.ViewController'
	,requires: [

	]
	,alias: 'controller.speedcloud.pipeline.jenkins.JenkinsAdapterController'

    ,onAddButtonClick: function() {

        var modelConfig = {}
        var record = Ext.create('AM.model.speedcloud.pipeline.jenkins.JenkinsAdapter', modelConfig);
                ...
                store.loadPage(targetPage,{
                    ...
                    ,callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.notification('操作成功','删除JenkinsAdapter成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onSimpleSearchButtonClick: function(button, e, options) {
        ...
        var projectField = me.lookupReference("projectField");
        searchWindow.down('#projectField').setValue(projectField.getValue());

        var envField = me.lookupReference("envField");
        searchWindow.down('#envField').setValue(envField.getValue());
    ...
    ,onExportButtonClick: function(button, e, options) {
        ...
        Ext.Ajax.request({
            disableCaching: true 
            ,url: "speedcloud/pipeline/jenkins/jenkinsadapter/export"
            ,method: "POST"
