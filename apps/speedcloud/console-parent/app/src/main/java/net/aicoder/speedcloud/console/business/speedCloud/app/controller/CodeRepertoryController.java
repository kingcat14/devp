...
import net.aicoder.speedcloud.console.business.speedCloud.app.service.CodeRepertoryRibbonService;
import net.aicoder.speedcloud.console.business.speedCloud.app.valid.CodeRepertoryValidator;
...
public class CodeRepertoryController {
	...
	@Autowired
	private CodeRepertoryRibbonService codeRepertoryRibbonService;

	@Autowired
	CodeRepertoryValidator codeRepertoryValidator;
	...
	public CodeRepertoryVO add(@RequestBody CodeRepertoryAddDto codeRepertoryAddDto){
    	codeRepertoryAddDto.setTid(saaSUtil.getAccount().getTenantId());
		return  codeRepertoryRibbonService.add(codeRepertoryAddDto);
	}
	...
	public void delete(@PathVariable String idArray){
		...
		for (String id : ids ){
			codeRepertoryRibbonService.delete(Long.parseLong(id));
		}
	...
	@PutMapping(value="/{id}")
	public CodeRepertoryVO update(@RequestBody CodeRepertoryEditDto codeRepertoryEditDto, @ApiParam(value = "要查询的代码库id") @PathVariable Long id){

		CodeRepertoryVO vo = codeRepertoryRibbonService.merge(id, codeRepertoryEditDto);
	...
	@GetMapping(value="/{id}")
	public CodeRepertoryVO get(@ApiParam(value = "要查询的代码库id") @PathVariable Long id) {

		CodeRepertoryVO vo = codeRepertoryRibbonService.find(id);
		return vo;
	}
	...
	public PageContent<CodeRepertoryVO> list(@RequestBody PageSearchRequest<CodeRepertoryCondition> pageSearchRequest){
		...
		CodeRepertoryCondition condition = pageSearchRequest.getSearchCondition();
		if(condition==null){
			condition = new CodeRepertoryCondition();
			pageSearchRequest.setSearchCondition(condition);
		}
        pageSearchRequest.getSearchCondition().setTid(saaSUtil.getAccount().getTenantId());
		PageContent<CodeRepertoryVO> pageContent = codeRepertoryRibbonService.list(pageSearchRequest);
		for(CodeRepertoryVO vo : pageContent.getContent()){
			initViewProperty(vo);
		}
	...
	private CodeRepertoryVO initViewProperty( CodeRepertoryVO vo){

	   

		SimpleConfig typeSimpleConfig = simpleConfigService.findByConfigTypeAndCode("CODEREPERTORY-TYPE", vo.getType());
