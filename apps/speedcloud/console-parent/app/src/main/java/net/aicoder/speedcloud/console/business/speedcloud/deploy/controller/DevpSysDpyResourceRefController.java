...
import net.aicoder.speedcloud.console.business.speedcloud.deploy.valid.DevpSysDpyResourceRefValidator;
...
public class DevpSysDpyResourceRefController {
	...
	@Autowired
	private DevpSysDpyResourceRefRibbonService devpSysDpyResourceRefRibbonService;

	@Autowired
	DevpSysDpyResourceRefValidator devpSysDpyResourceRefValidator;
	...
	public DevpSysDpyResourceRefVO add(@RequestBody DevpSysDpyResourceRefAddDto devpSysDpyResourceRefAddDto){
    	devpSysDpyResourceRefAddDto.setTid(saaSUtil.getAccount().getTenantId());
		return  devpSysDpyResourceRefRibbonService.add(devpSysDpyResourceRefAddDto);
	}
	...
	public void delete(@PathVariable String idArray){
		...
		for (String id : ids ){
			devpSysDpyResourceRefRibbonService.delete(Long.parseLong(id));
		}
	...
	@PutMapping(value="/{id}")
	public DevpSysDpyResourceRefVO update(@RequestBody DevpSysDpyResourceRefEditDto devpSysDpyResourceRefEditDto, @ApiParam(value = "要查询的方案资源间关系id") @PathVariable Long id){

		DevpSysDpyResourceRefVO vo = devpSysDpyResourceRefRibbonService.merge(id, devpSysDpyResourceRefEditDto);
	...
	@GetMapping(value="/{id}")
	public DevpSysDpyResourceRefVO get(@ApiParam(value = "要查询的方案资源间关系id") @PathVariable Long id) {

		DevpSysDpyResourceRefVO vo = devpSysDpyResourceRefRibbonService.find(id);
		return vo;
	}
	...
	public PageContent<DevpSysDpyResourceRefVO> list(@RequestBody PageSearchRequest<DevpSysDpyResourceRefCondition> pageSearchRequest){
		...
		DevpSysDpyResourceRefCondition condition = pageSearchRequest.getSearchCondition();
		if(condition==null){
			condition = new DevpSysDpyResourceRefCondition();
			pageSearchRequest.setSearchCondition(condition);
		}
        pageSearchRequest.getSearchCondition().setTid(saaSUtil.getAccount().getTenantId());
		PageContent<DevpSysDpyResourceRefVO> pageContent = devpSysDpyResourceRefRibbonService.list(pageSearchRequest);
		for(DevpSysDpyResourceRefVO vo : pageContent.getContent()){
			initViewProperty(vo);
		}
	...
	private DevpSysDpyResourceRefVO initViewProperty( DevpSysDpyResourceRefVO vo){
