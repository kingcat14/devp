...
public class ModelTypeValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
	    if(obj instanceof ModelTypeAddDto){
            this.validateModelTypeAddDto((ModelTypeAddDto)obj, errors);
        }
    ...
    public void validateSearchDto(PageSearchRequest<ModelTypeCondition> search, Errors errors) {
        if(search.getSearchCondition() == null){
            search.setSearchCondition(new ModelTypeCondition());
        }
    }
     ...
     * @param errors
     */
	public void validateModelTypeAddDto(ModelTypeAddDto modelType, Errors errors) {
		...
		if(StringUtils.length(modelType.getCode()) > 255){
			errors.rejectValue(ModelType.PROPERTY_CODE,null,"code最长255个字符");
		}
		if(StringUtils.length(modelType.getName()) > 255){
			errors.rejectValue(ModelType.PROPERTY_NAME,null,"name最长255个字符");
		}
	}
}