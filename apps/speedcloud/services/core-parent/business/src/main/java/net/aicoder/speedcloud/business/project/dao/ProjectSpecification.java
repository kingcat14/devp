...
public class ProjectSpecification implements Specification<Project>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<Project> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(Project.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(Project.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
