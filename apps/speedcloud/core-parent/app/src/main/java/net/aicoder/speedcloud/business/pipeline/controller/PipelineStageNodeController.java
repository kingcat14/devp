...
@RestController
@RequestMapping(value = "/pipeline/pipelineStageNode")
public class PipelineStageNodeController {
	...
	public PipelineStageNodeVO add(@RequestBody @Valid PipelineStageNodeAddDto pipelineStageNodeAddDto){
		...
		pipelineStageNode.setTid(saaSUtil.getAccount().getTenantId());
	...
	private PipelineStageNodeVO initViewProperty(PipelineStageNode pipelineStageNode){
		...
		SimpleConfig nodeTypeSimpleConfig = simpleConfigService.findByConfigTypeAndCode("PIPELINESTAGENODE-NODETYPE", 阶段执行节点.getNodeType());
