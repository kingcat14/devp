...
public class SchemeValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
	    if(obj instanceof SchemeAddDto){
            this.validateSchemeAddDto((SchemeAddDto)obj, errors);
        }
    ...
    public void validateSearchDto(PageSearchRequest<SchemeCondition> search){
        if(search.getSearchCondition() == null){
            search.setSearchCondition(new SchemeCondition());
        }
    }
     ...
     * @param errors
     */
	public void validateSchemeAddDto(SchemeAddDto scheme, Errors errors) {
		...
		if(StringUtils.length(scheme.getName()) > 255){
			errors.rejectValue(Scheme.PROPERTY_NAME,null,"方案名称最长255个字符");
		}
		if(StringUtils.length(scheme.getCode()) > 255){
			errors.rejectValue(Scheme.PROPERTY_CODE,null,"方案代码最长255个字符");
		}
		if(StringUtils.length(scheme.getAlias()) > 255){
			errors.rejectValue(Scheme.PROPERTY_ALIAS,null,"方案别名最长255个字符");
		}
		if(StringUtils.length(scheme.getType()) > 255){
			errors.rejectValue(Scheme.PROPERTY_TYPE,null,"方案类型最长255个字符");
		}
		if(StringUtils.length(scheme.getVersion()) > 255){
			errors.rejectValue(Scheme.PROPERTY_VERSION,null,"版本最长255个字符");
		}
		if(StringUtils.length(scheme.getVerPostfix()) > 255){
			errors.rejectValue(Scheme.PROPERTY_VER_POSTFIX,null,"版本标识后缀最长255个字符");
		}
		if(StringUtils.length(scheme.getNotes()) > 255){
			errors.rejectValue(Scheme.PROPERTY_NOTES,null,"备注最长255个字符");
		}
	}
}