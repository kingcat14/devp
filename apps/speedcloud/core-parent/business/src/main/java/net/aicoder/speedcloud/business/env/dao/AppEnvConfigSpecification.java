...
public class AppEnvConfigSpecification implements Specification<AppEnvConfig>{
	...
	private void tryAddTidPredicate(List<Predicate> predicateList, Root<AppEnvConfig> root, CriteriaBuilder cb){
		...
		if (null != condition.getTidMax() ) {
			predicateList.add(cb.greaterThanOrEqualTo(root.get(AppEnvConfig.PROPERTY_TID).as(Long.class), condition.getTidMax()));
		}

		if (null != condition.getTidMin() ) {
			predicateList.add(cb.lessThan(root.get(AppEnvConfig.PROPERTY_TID).as(Long.class), condition.getTidMin()));
		}
	}
