...
public class DevpSysDpyResourcesSpecification implements Specification<DevpSysDpyResources>{
	...
	private void tryAddCategoryPredicate(List<Predicate> predicateList, Root<DevpSysDpyResources> root, CriteriaBuilder cb){
	    if (null != condition.getCategory() ) {
            predicateList.add(cb.equal(root.get(DevpSysDpyResources.PROPERTY_CATEGORY).as(Long.class), condition.getCategory()));
        }
	}
	private void tryAddTypePredicate(List<Predicate> predicateList, Root<DevpSysDpyResources> root, CriteriaBuilder cb){
	    if (null != condition.getType() ) {
            predicateList.add(cb.equal(root.get(DevpSysDpyResources.PROPERTY_TYPE).as(Long.class), condition.getType()));
        }
	}
	...
	private void tryAddEvnPredicate(List<Predicate> predicateList, Root<DevpSysDpyResources> root, CriteriaBuilder cb){
	    if (null != condition.getEvn() ) {
            predicateList.add(cb.equal(root.get(DevpSysDpyResources.PROPERTY_EVN).as(Long.class), condition.getEvn()));
        }
	}
	...
	private void tryAddProjectPredicate(List<Predicate> predicateList, Root<DevpSysDpyResources> root, CriteriaBuilder cb){
	    if (null != condition.getProject() ) {
            predicateList.add(cb.equal(root.get(DevpSysDpyResources.PROPERTY_PROJECT).as(Long.class), condition.getProject()));
        }
	}
	private void tryAddOuterResourcePredicate(List<Predicate> predicateList, Root<DevpSysDpyResources> root, CriteriaBuilder cb){
