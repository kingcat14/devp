Ext.define('AM.view.maintenance.rdc.config.DevelopTypePanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'maintenance.rdc.config.DevelopTypePanel'
    ,title: '开发模式'
    ,requires: [
        'AM.view.maintenance.rdc.config.DevelopTypeController'
        ,'AM.view.maintenance.rdc.config.DevelopTypeAddWindow'
        ,'AM.view.maintenance.rdc.config.DevelopTypeEditWindow'
        ,'AM.view.maintenance.rdc.config.DevelopTypeSearchWindow'
        ,'AM.view.maintenance.rdc.config.DevelopTypeDetailWindow'
    ]
    ...
    ,initComponent: function() {
        ...
        me.add({xtype:'maintenance.rdc.config.DevelopTypeAddWindow',reference:'developTypeAddWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'maintenance.rdc.config.DevelopTypeEditWindow',reference:'developTypeEditWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'maintenance.rdc.config.DevelopTypeSearchWindow',reference:'developTypeSearchWindow',listeners:{saved:'doSearch'}})
        me.add({xtype:'maintenance.rdc.config.DevelopTypeDetailWindow',reference:'developTypeDetailWindow'})
    ...
    ,onSimpleSearchButtonClick: function(button, e, options) {
        ...
        var searchCondition = {:simpleSearchField.getValue()}
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.maintenance.rdc.config.DevelopType', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新开发模式');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改开发模式信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除开发模式成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改开发模式信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        var me = this;
        var addWindow = me.lookupReference('developTypeAddWindow');
    ...
    ,showEditWindow: function(model, targetComponent) {
        var me = this;
        var editWindow = me.lookupReference('developTypeEditWindow');
    ...
    ,showDetailWindow: function(model, targetComponent) {
        var me = this;
        var detailWindow = me.lookupReference('developTypeDetailWindow');
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        var searchWindow = me.lookupReference('developTypeSearchWindow');
