Ext.define('AM.view.maintenance.config.SimpleConfigPanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'maintenance.config.SimpleConfigPanel'
    ,title: '通用配置'
    ,requires: [
        'AM.view.maintenance.config.SimpleConfigController'
        ,'AM.view.maintenance.config.SimpleConfigAddWindow'
        ,'AM.view.maintenance.config.SimpleConfigEditWindow'
        ,'AM.view.maintenance.config.SimpleConfigSearchWindow'
        ,'AM.view.maintenance.config.SimpleConfigDetailWindow'
    ]
    ...
    ,initComponent: function() {
        ...
        me.add({xtype:'maintenance.config.SimpleConfigAddWindow',reference:'simpleConfigAddWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'maintenance.config.SimpleConfigEditWindow',reference:'simpleConfigEditWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'maintenance.config.SimpleConfigSearchWindow',reference:'simpleConfigSearchWindow',listeners:{saved:'doSearch'}})
        me.add({xtype:'maintenance.config.SimpleConfigDetailWindow',reference:'simpleConfigDetailWindow'})
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.maintenance.config.SimpleConfig', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新通用配置');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改通用配置信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除通用配置成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改通用配置信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        var me = this;
        var addWindow = me.lookupReference('simpleConfigAddWindow');
    ...
    ,showEditWindow: function(model, targetComponent) {
        var me = this;
        var editWindow = me.lookupReference('simpleConfigEditWindow');
    ...
    ,showDetailWindow: function(model, targetComponent) {
        var me = this;
        var detailWindow = me.lookupReference('simpleConfigDetailWindow');
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        var searchWindow = me.lookupReference('simpleConfigSearchWindow');
