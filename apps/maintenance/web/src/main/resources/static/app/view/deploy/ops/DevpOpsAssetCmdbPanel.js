Ext.define('AM.view.deploy.ops.DevpOpsAssetCmdbPanel', {
    extend: 'Ext.grid.Panel'
    ,xtype: 'deploy.ops.DevpOpsAssetCmdbPanel'
    ,title: '资产定义'
    ,requires: [
        'AM.view.deploy.ops.DevpOpsAssetCmdbController'
        ,'AM.view.deploy.ops.DevpOpsAssetCmdbAddWindow'
        ,'AM.view.deploy.ops.DevpOpsAssetCmdbEditWindow'
        ,'AM.view.deploy.ops.DevpOpsAssetCmdbSearchWindow'
        ,'AM.view.deploy.ops.DevpOpsAssetCmdbDetailWindow'
    ]
    ...
    ,initComponent: function() {
        ...
        me.add({xtype:'deploy.ops.DevpOpsAssetCmdbAddWindow',reference:'devpOpsAssetCmdbAddWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'deploy.ops.DevpOpsAssetCmdbEditWindow',reference:'devpOpsAssetCmdbEditWindow',listeners:{saved:'reloadStore'}})
        me.add({xtype:'deploy.ops.DevpOpsAssetCmdbSearchWindow',reference:'devpOpsAssetCmdbSearchWindow',listeners:{saved:'doSearch'}})
        me.add({xtype:'deploy.ops.DevpOpsAssetCmdbDetailWindow',reference:'devpOpsAssetCmdbDetailWindow'})
	...
	,onAddButtonClick: function(button, e, options) {
        ...
        var record = Ext.create('AM.model.deploy.ops.DevpOpsAssetCmdb', modelConfig);

        options.src = button;
        var addWindow = this.showAddWindow(record, button);
        addWindow.setTitle('添加新资产定义');
    }
    ,onEditButtonClick: function(button, e, options) {
        ...
        detailWindow.setTitle('修改资产定义信息');
    }
                ...
                store.loadPage(targetPage,{
                    ...
                    callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.show('操作成功','删除资产定义成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onItemDblClick: function(tablepanel, record, item, index, e, options) {
        ...
        editWindow.setTitle('修改资产定义信息');
    }
    ,showAddWindow: function(model, targetComponent) {
        var me = this;
        var addWindow = me.lookupReference('devpOpsAssetCmdbAddWindow');
    ...
    ,showEditWindow: function(model, targetComponent) {
        var me = this;
        var editWindow = me.lookupReference('devpOpsAssetCmdbEditWindow');
    ...
    ,showDetailWindow: function(model, targetComponent) {
        var me = this;
        var detailWindow = me.lookupReference('devpOpsAssetCmdbDetailWindow');
    ...
    ,showSearchWindow: function(button, e, options) {
        ...
        var searchWindow = me.lookupReference('devpOpsAssetCmdbSearchWindow');
