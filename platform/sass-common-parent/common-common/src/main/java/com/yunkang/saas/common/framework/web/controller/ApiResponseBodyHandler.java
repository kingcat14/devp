...
@ControllerAdvice class ApiResponseBodyHandler implements ResponseBodyAdvice<Object> {
	...
	public Object beforeBodyWrite(Object object, MethodParameter returnType, MediaType selectedContentType,
		...
		if(response instanceof ServletServerHttpResponse){
			...
			if(HttpStatus.OK.value() != servletResponse.getServletResponse().getStatus()){
				result.setCode(servletResponse.getServletResponse().getStatus());
				result.setMessage(HttpStatus.valueOf(servletResponse.getServletResponse().getStatus()).getReasonPhrase());
//				result.setData();
			}
		}
		...
		if (object instanceof RestResponse) {
			...
			result.setCode(RestStatus.ERROR.code);
