Ext.define('AM.view.monitor.supporter.SupporterAppRelationController', {
	extend: 'Ext.app.ViewController'
	,requires: [

	]
	,alias: 'controller.monitor.supporter.SupporterAppRelationController'

	,onAddButtonClick: function() {

        var modelConfig = {}
        var record = Ext.create('AM.model.monitor.supporter.SupporterAppRelation', modelConfig);
                ...
                store.loadPage(targetPage,{
                    ...
                    ,callback: function(records, operation, success) {
                        if(!success)
                        	...
                        	Ext.MsgUtil.notification('操作成功','删除支持应用成功!');
                    }
                });
            }
            ,scope:me.getStore()
    ...
    ,onSimpleSearchButtonClick: function(button, e, options) {
        ...
        var supporterField = me.lookupReference("supporterField");
        searchWindow.down('#supporterField').setValue(supporterField.getValue());

        var applicationField = me.lookupReference("applicationField");
        searchWindow.down('#applicationField').setValue(applicationField.getValue());
    ...
    ,onExportButtonClick: function(button, e, options) {
        ...
        Ext.Ajax.request({
            disableCaching: true 
            ,url: "monitor/supporter/supporterapprelation/export"
            ,method: "POST"
